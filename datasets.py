...
from utils import images_pca, images_pca_v2
...
class ImageDataset_kaggle_v2(Dataset):
    def __init__(self, root, transforms_=None, unaligned=False, mode='train'):
        self.transform = transforms.Compose(transforms_)
        self.unaligned = unaligned

        self.files_A = sorted(glob.glob(os.path.join(root, 'photo_jpg') + '/*.*'))
        self.files_B = sorted(images_pca_v2(glob.glob(os.path.join(root, 'monet_jpg') + '/*.*')))
