...
def images_pca_v2(images,k=30):
    resize = transforms.Resize(256)
    data = np.zeros((len(images),256*256))
    idx = 0
    for img in images:
        data[idx] = np.array(resize(Image.open(img)).convert('L')).reshape(1,-1)
        idx = idx+1
    pca = PCA(2)
    ...
    save_reduced_images_v2(reduced_images)
    print(len(reduced_images))
    return reduced_images

def save_reduced_images_v2(images):
    os.makedirs('/content/Pnina/MyDrive/Cycle_Gan/kaggle_dataset/monet_reduced', exist_ok=True)
    i = 0
    for img in images:
        Image.open(img).convert('RGB').save('/content/Pnina/MyDrive/Cycle_Gan/kaggle_dataset/monet_reduced/'+str(i).zfill(3)+'.png')
        i=i+1
